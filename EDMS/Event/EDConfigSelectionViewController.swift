//
//  EDConfigSelectionViewController.swift
//  EDMS
//
...
class EDConfigSelectionViewController: UIViewController {
    ...
    lazy var quantityView: EDQuantitySelectorView = {
        let view = EDQuantitySelectorView()
        return view
    }()
    ...
    @objc func addToCart() {
        let newBill = BillRequest(id: 0, comId: com.id, quantity: quantityView.currentQuantity, optionId: com.options[shoppingCollectionView.selectedRow].id, orderId: EDUser.user.cart)
        EDUser.addToCart(bill: newBill) { _ in
