...
class EDTourView: UITableViewCell, UITableViewDataSource, UITableViewDelegate {
    var event = Event()
    var schedules: [[Game]] = []
...
//        addSubview(drawView)
    ...
    override func layoutSubviews() {
        ...
        liveGameView.register(EDEventGameCell.self, forCellReuseIdentifier: "EDEventGameCell")
    ...
    func setupEvent(_ event: Event) {
        ...
        if Date().timeIntervalSince1970 > event.startDate, Date().timeIntervalSince1970 < event.endDate {
            ...
            if EDUser.user.allEvents.contains(where: { $0 == event.id }) {
                joinConfig = TMButtonConfig(title: NSLocalizedString("ONGOING", comment: ""), action: nil, actionTarget: self)
            }
        }
        let tourSchedules = EDDataConvert.datesInRangeString(startDate: event.startDate, endDate: event.endDate)
    ...
    func tableView(_: UITableView, numberOfRowsInSection _: Int) -> Int {
        schedules = EDDataConvert.union(event.schedule, to: schedules)
