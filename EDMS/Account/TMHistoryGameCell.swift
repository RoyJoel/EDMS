...
class EDHistoryGameCell: UITableViewCell {
    ...
    func setupEvent(game: Game) {
        opponentIcon.image = UIImage(data: game.player1.id == EDUser.user.id ? game.player2.icon.toPng() : game.player1.icon.toPng())
        opponentLabel.text = game.player1.id == EDUser.user.id ? game.player2.name : game.player1.name
        dateLabel.text = game.startDate.convertToString(formatterString: "yyyy MM-dd HH:mm")
        placeLabel.text = game.place
        let liveResult = EDDataConvert.gameResult(from: game.result, isGameCompleted: false)
        var lastRecord = ""
