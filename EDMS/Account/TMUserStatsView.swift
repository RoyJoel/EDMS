...
class EDUserStatsView: UIView {
    ...
    lazy var firstServeInView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var firstServeWonView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var secondServeInView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var secondServeWonView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var returnFirstServeInView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var returnFirstServeWonView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var returnSecondServeInView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var returnSecondServeWonView: EDCircleStatsView = {
        let circleStatsView = EDCircleStatsView()
        return circleStatsView
    }()

    lazy var breakPointSavTMView: EDCircleStatsView = {
        let infoView = EDCircleStatsView()
        return infoView
    }()

    lazy var breakPointConvertTMView: EDCircleStatsView = {
        let infoView = EDCircleStatsView()
        return infoView
    }()

    lazy var serveGameWonView: EDCircleStatsView = {
        let infoView = EDCircleStatsView()
        return infoView
    }()

    lazy var returnServeGameWonView: EDCircleStatsView = {
        let infoView = EDCircleStatsView()
        return infoView
    }()

    lazy var servePointWonView: EDCircleStatsView = {
        let infoView = EDCircleStatsView()
        return infoView
    }()

    lazy var returnServePointWonView: EDCircleStatsView = {
        let infoView = EDCircleStatsView()
        return infoView
    }()
    ...
    func setupUI() {
        ...
        backgroundView.addSubview(breakPointSavTMView)
        backgroundView.addSubview(breakPointConvertTMView)
        ...
        if UIScreen.main.nativeBounds.height < 2360 {
            ...
            breakPointSavTMView.snp.makeConstraints { make in
            ...
            serveGameWonView.snp.makeConstraints { make in
                make.top.equalTo(breakPointSavTMView.snp.top)
                make.left.equalTo(breakPointSavTMView.snp.right).offset(8)
                make.height.equalTo(92)
                make.width.equalTo(110)
            }
            ...
            breakPointConvertTMView.snp.makeConstraints { make in
            ...
            returnServeGameWonView.snp.makeConstraints { make in
                make.top.equalTo(breakPointConvertTMView.snp.top)
                make.right.equalTo(breakPointConvertTMView.snp.left).offset(-8)
                make.height.equalTo(92)
                make.width.equalTo(110)
            }
        ...
        if UIScreen.main.nativeBounds.height < 2360 {
            ...
            breakPointSavTMView.setUpUI(sideLen: 40, insight: 4, title: NSLocalizedString("BREAK POINT SAVED", comment: ""))
            breakPointConvertTMView.setUpUI(sideLen: 40, insight: 4, title: NSLocalizedString("BREAK POINT CONVERT", comment: ""))
            ...
            breakPointSavTMView.setUpUI(sideLen: 50, insight: 12, title: NSLocalizedString("BREAK POINT SAVED", comment: ""))
            breakPointConvertTMView.setUpUI(sideLen: 50, insight: 12, title: NSLocalizedString("BREAK POINT CONVERT", comment: ""))
    ...
    func setupEvent(stats: Stats) {
        ...
        breakPointSavTMView.startAnimation(radii: breakPointSaved)
        breakPointConvertTMView.startAnimation(radii: breakPointConvert)
...
extension EDUserStatsView: JXSegmentedListContainerViewListDelegate {
