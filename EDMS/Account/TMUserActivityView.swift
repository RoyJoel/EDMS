...
class EDUserActivityView: UIView {
    lazy var leftActivityView: EDGameStatsView = {
        let view = EDGameStatsView()
        return view
    }()

    lazy var midActivityView: EDGameStatsView = {
        let view = EDGameStatsView()
        return view
    }()

    lazy var rightActivityView: EDGameStatsView = {
        let view = EDGameStatsView()
        return view
    }()
    ...
    func setupAlart() {
        ...
        let lastGameTime = UserDefaults.standard.double(forKey: EDUDKeys.lastGameTime.rawValue)
        if lastGameTime == 0 {
...
extension EDUserActivityView: JXSegmentedListContainerViewListDelegate {
